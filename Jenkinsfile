            ...
            steps {
                script {
                    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-key']]) {
                        ...
                        bat """
                        powershell -Command "\$accFrontendUrl = '${acc_front_service_url}'; (Get-Content 'E:\\docker_dev\\acc_react_back_cloud\\src\\main\\resources\\application.yaml') -replace 'FRONTEND_SERVICE_URL:.*', 'FRONTEND_SERVICE_URL: http://\$accFrontendUrl' | Set-Content 'E:\\docker_dev\\acc_react_back_cloud\\src\\main\\resources\\application.yaml';"
                        """
                    }
                }
            }
